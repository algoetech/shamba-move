...
<x-front>
    ...
    <div class="fixed top-0 w-full">
        ...
        <img class="w-full" id="banner"
            src="{{ $page->banner ? url($page->banner) : asset('assets/images/villages-farm.jpg') }}" alt="Farmer">
    </div>
    ...
    <section class="container relative w-full p-0 mt-20 align-middle rounded-2 z-100 ">
        <h1 class="pt-20 pb-10 font-bold text-center text-7xl text-vendor-secondary-beta tai-font">{{ $page->title }}
        </h1>
    </section>
                    ...
                    @foreach ($pages as $nopage)
                        @if (!$nopage->resource)
                            <li class="flex items-center px-3 py-2 space-x-3 hover:bg-green-500/20 rounded-1 rtl:space-x-reverse wow flipInY"
                                data-wow-duration="1s" data-wow-delay="{{ ($loop->index + 1) * 0.6 }}s">
                                ...
                                <span class="capitalize ">
                                    <a class="hover:scale-110 hover:text-vendor-secondary-alpha text-vendor-compliment-primary"
                                        href="{{ route('frontend.page', ['slug' => $nopage->slug]) }}">
                                        {{ $nopage->title }}
                                    </a>
                                </span>
                            </li>
