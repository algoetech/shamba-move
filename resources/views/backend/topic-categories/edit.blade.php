        ...
        <div class="p-4 md:p-5">
            <form class="space-y-4" action="{{route('admin.topic-categories.update', $topicsCategories->id)}}" method="POST" enctype="multipart/form-data">
                ...
                <div>
                    ...
                    <div id="editor" class="flex h-32 w-[100%]">{!! $topicsCategories->description !!}</div>
                    <textarea name="description" id="contentInput" rows="4" class="hidden bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="Enter user bio">{!! $topicsCategories->description !!}</textarea>
                </div>

                <button type="submit" class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Update topicCategories</button>
            </form>
