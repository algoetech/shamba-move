            ...
            <table class="w-full text-sm text-left text-gray-500 rtl:text-right dark:text-gray-400">
                ...
                <tbody>
                    ...
                    <tr
                        ...
                        <td class="px-6 py-4">
                            {!! Str::limit($page->content, 70, '...') !!}
                        </td>
                        <td class="px-6 py-4">
                            <span class="bg-{{ $page->resource?'green':'rose' }}-100 text-{{ $page->resource?'green':'rose' }}-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-{{ $page->resource?'green':'rose' }}-900 dark:text-{{ $page->resource?'green':'rose' }}-300">{{ $page->resource?'True':'False' }}</span>
                            <span class="bg-green-100 text-green-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300 hidden">Green</span>
                            <span class="bg-rose-100 text-rose-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-rose-900 dark:text-rose-300 hidden">Green</span>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex flex-row gap-2 ">
                                ...
                                <form action="{{route('admin.pages.destroy', $page->id)}}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <input type="hidden" name="userid" value="{{$page->id}}" />
