    ...
    <div class="relative w-full max-h-full p-4 lg:max-w-[42rem] md:max-w-xl">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-4 md:p-5">
              <form class="space-y-4" action="{{ route('admin.posts.store') }}" method="POST" enctype="multipart/form-data">
                  @csrf
                  @method('POST')
                  <div>
                      <label for="title" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">post Title</label>
                      <input type="text" name="name" id="title" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="About Us" value="{{ old('name') }}" required />
                      @error('name')
                  ...
                  <div class="w-full grid grid-cols-12">
                    <div class="relative z-0 w-full px-5 mb-5 group col-span-full">
                        <input type="checkbox" name="isquestion" id="ispquestion" class="block py-2.5 px-0 w-full text-sm text-green-600 md:mt-[30px] bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
                        <label for="isquestion" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 md:mt-[10px] duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Is a Question</label>
                    </div>
                  </div>
